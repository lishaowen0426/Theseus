...
static COMPONENT_NAMESPACE: Once<Arc<CrateNamespace>> = Once::new(); 
...
pub fn create_component_namespace(recursive_namespace: Option<Arc<CrateNamespace>>) -> Result<&'static Arc<CrateNamespace>, &'static str>{
    // (1) use the initial kernel CrateNamespace as the new app namespace's recursive namespace if none was provided.
    let recursive_namespace = recursive_namespace
        .or_else(|| get_initial_kernel_namespace().cloned())
        .ok_or("initial kernel CrateNamespace not yet initialized")?;

    let component_namespace_name = CrateType::Component.default_namespace_name().to_string(); 
    let root_namespace_dir = get_namespaces_directory().ok_or("Couldn't find the root namespace directory")?;
    let component_namespace_dir = VFSDirectory::create(component_namespace_name.clone(), &root_namespace_dir)?;

    Ok(COMPONENT_NAMESPACE.call_once(|| Arc::new(CrateNamespace::new(component_namespace_name, NamespaceDir::new(component_namespace_dir), Some(recursive_namespace)))))
