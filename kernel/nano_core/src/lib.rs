    ...
    T: boot_info::BootInformation,
{
    ...
    println_raw!(
        "nano_core(): bootloader-provided RSDP address: {:X?}",
        rsdp_address
    );
    ...
    let (nano_core_crate_ref, initial_tls_image, ap_realmode_begin, ap_realmode_end, ap_gdt) =
        match mod_mgmt::parse_nano_core::parse_nano_core(
            ...
            Ok(NanoCoreItems {
                ...
                println_raw!(
                    "nano_core(): finished parsing the nano_core crate, {} new symbols.",
                    num_new_symbols
                );
