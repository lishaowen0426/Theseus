...
use memory::{MappedPages, PteFlags, VirtualAddress};
use spin::{Mutex, Once, RwLock};

pub use crate_metadata_serde::{
    SectionType, Shndx, BSS_SECTION_NAME, DATA_SECTION_NAME, EH_FRAME_SECTION_NAME,
    GCC_EXCEPT_TABLE_SECTION_NAME, RODATA_SECTION_NAME, TEXT_SECTION_NAME, TLS_BSS_SECTION_NAME,
...
pub const TEXT_SECTION_FLAGS: PteFlags = PteFlags::from_bits_truncate(
    (PteFlags::new().bits() | PteFlags::VALID.bits()) & !PteFlags::NOT_EXECUTABLE.bits(), // clear the no-exec bits
);
...
const _: () =
    assert!(DATA_BSS_SECTION_FLAGS.is_writable() && !DATA_BSS_SECTION_FLAGS.is_executable());
...
impl CrateType {
    ...
    pub fn default_namespace_name(&self) -> &'static str {
        match self {
            ...
            CrateType::Component => "_component",
        }
    }
    ...
    pub fn from_module_name(module_name: &str) -> Result<(CrateType, &str, &str), &'static str> {
        ...
        } else if prefix.starts_with(CrateType::Component.first_char()) {
            Ok((CrateType::Component, namespace_prefix, crate_name))
        } else {
            error!(
...
impl LoadedCrate {
    ...
    pub fn get_function_section(&self, func_name: &str) -> Option<&StrongSectionRef> {
        self.find_section(|sec| sec.typ == SectionType::Text && sec.name.as_str() == func_name)
    }
    ...
    {
        self.sections.values().find(|&sec| predicate(sec))
    }
    ...
    pub fn deep_copy(
            ...
            |old_mp_range: &(Arc<Mutex<MappedPages>>, Range<VirtualAddress>),
             ...
             -> Result<(Arc<Mutex<MappedPages>>, Range<VirtualAddress>), &'static str> {
                ...
                let new_mp = old_mp_range
                    .0
                    .lock()
                    .deep_copy(page_table, Some(flags.writable(true)))?;
        ...
        for new_sec in new_sections.values() {
            ...
            let new_sec_slice: &mut [u8] =
                new_sec_mapped_pages.as_slice_mut(0, new_sec_mapped_pages_offset + new_sec.size)?;
...
pub fn section_name_str_ref(section_type: &SectionType) -> StrRef {
    ...
    instance
        .call_once(|| StrRef::from(section_type.name()))
        .clone()
}
...
impl fmt::Display for LoadedSection {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            self.name, self.typ, self.virt_addr, self.size,
        )
    }
}
...
impl fmt::Debug for LoadedSection {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(parent_crate_name) = self
            .parent_crate
            .upgrade()
            .and_then(|cref| cref.try_lock_as_ref().map(|c| c.crate_name.clone()))
        {
    ...
    match relocation_entry.typ {
        R_X86_64_32 => {
            ...
            let source_val = source_sec_vaddr
                .value()
                .wrapping_add(relocation_entry.addend) as u32;
            if verbose_log {
                trace!("                    target_ptr: {:p}, source_val: {:#X} (from source_sec_vaddr {:#X})", target_ref.as_ptr(), source_val, source_sec_vaddr);
            }
        ...
        R_X86_64_64 => {
            ...
            let source_val = source_sec_vaddr
                .value()
                .wrapping_add(relocation_entry.addend) as u64;
            if verbose_log {
                trace!("                    target_ptr: {:p}, source_val: {:#X} (from source_sec_vaddr {:#X})", target_ref.as_ptr(), source_val, source_sec_vaddr);
            }
            target_ref.copy_from_slice(&source_val.to_ne_bytes());
        ...
        R_X86_64_PC32 | R_X86_64_PLT32 => {
            ...
            let source_val = source_sec_vaddr
                ...
                .wrapping_sub(target_ref.as_ptr() as usize) as u32;
            if verbose_log {
                trace!("                    target_ptr: {:p}, source_val: {:#X} (from source_sec_vaddr {:#X})", target_ref.as_ptr(), source_val, source_sec_vaddr);
            }
            target_ref.copy_from_slice(&source_val.to_ne_bytes());
        ...
        R_X86_64_PC64 => {
            ...
            let source_val = source_sec_vaddr
                .value()
                .wrapping_add(relocation_entry.addend)
                .wrapping_sub(target_ref.as_ptr() as usize);
            if verbose_log {
                trace!("                    target_ptr: {:p}, source_val: {:#X} (from source_sec_vaddr {:#X})", target_ref.as_ptr(), source_val, source_sec_vaddr);
            }
            target_ref.copy_from_slice(&source_val.to_ne_bytes());
        ...
        _ => {
            error!(
                "found unsupported relocation type {}\n    \
