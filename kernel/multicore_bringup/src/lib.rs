...
use apic::{bootstrap_cpu, cpu_count, current_cpu, get_lapics, has_x2apic, LocalApic};
use core::{borrow::BorrowMut, convert::TryInto, ops::DerefMut, sync::atomic::Ordering};
...
use memory::{MappedPages, MmiRef, PhysicalAddress, PteFlags, VirtualAddress};
...
pub fn handle_ap_cores(
    ...
    let mut ap_startup_mapped_pages: MappedPages; // must be held throughout APs being booted up
    {
        ...
        trampoline_mapped_pages =
            page_table.map_allocated_pages_to(trampoline_page, trampoline_frame, flags)?;
        ap_startup_mapped_pages =
            page_table.map_allocated_pages_to(ap_startup_pages, ap_startup_frames, flags)?;
    ...
    ap_trampoline_data.ap_gdt.write(
        ap_gdt
            .value()
            .try_into()
            .map_err(|_| "AP_GDT physical address larger than u32::MAX")?,
    );
    ...
    // Retrieve the graphic mode information written during the AP bootup sequence in `ap_realmode.asm`.
    {
        ...
        info!("graphic info: {:?}", graphic_info);
        *GRAPHIC_INFO.lock() = Some(*graphic_info);
    }
    {
        let info = get_graphic_info().unwrap();
        debug!("graphic info: {:?}", info);
    }
...
fn bring_up_ap(
    ...
    debug!("waiting 10 ms...");
    pit_clock_basic::pit_wait(10000).unwrap_or_else(|_e| {
    ...
    pit_clock_basic::pit_wait(300).unwrap_or_else(|_e| {
        error!("bring_up_ap(): failed to pit_wait 300 us. Error {:?}", _e);
    });
    pit_clock_basic::pit_wait(200).unwrap_or_else(|_e| {
